...
import type { BodyMeta, StaticProp } from '../api/types'
...
function drawIsoShadow(g: G, w: number, d: number, h: number): void {
  const hw = w / 2
  const hd = d / 2
...
function drawIsoChimney(g: G, w: number, d: number, h: number, count: number): void {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let s = 0; s < 3; s++) {
      const r = pseudoRandom(i * 17 + s * 3.3)
      g.fillStyle(0xd1d5db, 0.35 - s * 0.09)
      g.fillCircle(smokeBase.sx + (r - 0.5) * 10 + s * 3, smokeBase.sy - s * 10 - 4, 5 + s * 2)
    }
  }
}
...
function drawIsoFountain(g: G, w: number, d: number, h: number): void {
...
export function drawIsoProp(
  ...
  if (kind && _SHADOWED_KINDS.has(kind)) drawIsoShadow(g, w, d, h)
  switch (kind) {
    case 'house':
      drawIsoExtrudedBox(g, w, d, h, color)
      drawIsoRoof(g, w, d, h, shade(color, 0.65))
      return
    case 'apartment':
    case 'tower':
      drawIsoExtrudedBox(g, w, d, h, color)
      drawIsoWindows(g, w, d, h, seed)
      return
    case 'shop':
      drawIsoExtrudedBox(g, w, d, h, color)
    ...
    case 'factory':
      drawIsoExtrudedBox(g, w, d, h, color)
      drawIsoChimney(g, w, d, h, 1)
      return
    case 'power_plant':
      drawIsoExtrudedBox(g, w, d, h, color)
      drawIsoChimney(g, w, d, h, 2)
      return
    case 'road':
      drawIsoExtrudedBox(g, w, d, h, color)
    ...
    case 'water':
      ...
      return
    case 'tree':
      drawIsoTree(g, w, h, seed)
      return
    case 'fountain':
      drawIsoFountain(g, w, d, h)
      return
    default:
      drawIsoExtrudedBox(g, w, d, h, color)
  }

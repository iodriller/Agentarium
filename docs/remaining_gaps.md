...
| E5 | `pymunk2d/builder` | A body spawned deeply embedded in the ground (e.g. `create_body` with the schema-default `position: [0, 0]` on a 1×1 box, which engulfs the paper-thin `radius=0.1` ground segment) can tunnel through it forever instead of resting on top — observed `y -> -4412` by t=30s. Found via `mock_provider`'s own placeholder body (fixed: gave it `position: [0, 3]`, `IMPROVEMENTS.md` §8), but any agent (real LLM included) that places a body at/below ground level hits the same tunneling, silently scoring 0 with no error. | 🟡 Deferred: needs either a thicker/more robust ground collider or spawn-position validation; the system prompt already tells agents `y = height/2`, so residual risk is only careless/buggy tool calls |
...
| ID | Area | Issue | Status |
|----|------|-------|--------|
| L1 | `scoring_service` | `sorting_accuracy` / `city_score` rewards are rough proxies; real scoring would use actual physics outcomes. | 🟡 Deferred — `city_score` gained an infra-variety bonus (road/park/tree counts, height variety) in the 2026-07-05 city visual pass (`IMPROVEMENTS.md` §8), still a metrics proxy |
